...
h_fc_norm = tf.pow(tf.reduce_sum(tf.pow(h_fc1, 2), reduction_indices=1),0.5)
...
h_fc1_p_norm = tf.pow(tf.reduce_sum(tf.pow(h_fc1_p, 2), reduction_indices=1),0.5)
...
with tf.Session() as sess:
    ...
    f = open('log.txt', 'w+')
    for mini_batch_size in [75]:
        f.write('Mini Batch Size is: ' + str(mini_batch_size))

        for i in range(2500):

            batch = iF.generate_batch_pairs_from_preprocessed(images_match, images_mismatch, mini_batch_size, image_size, num_layer)
            ...
            if i % 100 == 0:
                ...
                save_path = saver.save(sess, "Batch_"+str(mini_batch_size)+"_training_"+str(i)+".ckpt")
